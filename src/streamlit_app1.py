...
def compute_prediction_performance(data_json: str) -> pd.DataFrame:
    ...
    columns = [
        "ticker","prediction_date","evaluation_date",
        "predicted_price","actual_price","error"
    ]
    perf_df = pd.DataFrame(records, columns=columns)
    if perf_df.empty:
        perf_df["absolute_error"] = pd.Series(dtype=float)
        perf_df["error_pct"] = pd.Series(dtype=float)
        return perf_df
...
def run_dashboard() -> None:
    ...
    if perf_df.empty or "ticker" not in perf_df.columns:
        st.info("No historical prediction data available yet.")
    else:
        ticker_perf_for_trend = perf_df.loc[perf_df["ticker"] == selected_ticker].copy()
